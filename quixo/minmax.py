...
class MinMaxPlayer(Player):
	...
	def make_move(self, game):
		...
		root = TreeNode(gamecopy, perspective_player=gamecopy.get_current_player())
...
class TreeNode:
	def __init__(self, game, perspective_player, action=None, parent=None):
		self.game = game
	...
	def expand(self):
		...
		for move in self.moves:
			...
			child = TreeNode(gamecopy, self.perspective_player, move, self)
	...
	def evaluate(self, game):
		...
		for row, column in zip(game.get_board(), transpose):
			...
			count.append(row_counter.get(self.perspective_player, 0))
			count.append(column_counter.get(self.perspective_player, 0))
			opponent_count.append(row_counter.get(1 - self.perspective_player, 0))
			opponent_count.append(column_counter.get(1 - self.perspective_player, 0))
		...
		count.append(main_diagonal_count.get(self.perspective_player, 0))
		count.append(second_diagonal_count.get(self.perspective_player, 0))
		opponent_count.append(main_diagonal_count.get(1 - self.perspective_player, 0))
		opponent_count.append(second_diagonal_count.get(1 - self.perspective_player, 0))
...
class Tree:
	def __init__(self, root, depth):
		self.root = root
		self.perspective_player = self.root.perspective_player
		self.depth = depth
